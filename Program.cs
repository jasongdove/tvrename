...
var folderArgument = new Argument<string>("folder", "The folder containing the media") { Arity = ArgumentArity.ExactlyOne };
...
rootCommand.SetHandler(async (string imdb, string? title, int? season, string folder) =>
{
    await RemuxLogic.Run(imdb, title, season, folder);
}, imdbOption, titleOption, seasonOption, folderArgument);
